...
from src.scanner.scanner import scan_apostas
from src.api.status import get_odds_api_status
from src.utils.utils import carregar_catalogo_ligas, TRADUCAO_ESPORTE_EN
...
async def banca_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if resumo["total_apostas"] == 0:
        ...
        if not bankroll_cfg:
            msg += (
                "\n\n💡 Banca não configurada. Para configurar: "
                "/banca <valor depositado> <valor da unidade>\n"
                "Exemplo: /banca 100 10"
            )
        await update.message.reply_text(msg, parse_mode='HTML')
        return
