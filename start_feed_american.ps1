...
if (-not $env:BOT_TOKEN_FEED_AMERICAN) {
    Write-Host "❌ BOT_TOKEN_FEED_AMERICAN não configurado!" -ForegroundColor Red
    Write-Host "Configure a variável de ambiente BOT_TOKEN_FEED_AMERICAN" -ForegroundColor Yellow
...
if (-not $env:ODDS_API_KEY) {
    Write-Host "❌ ODDS_API_KEY não configurada!" -ForegroundColor Red
    Write-Host "Configure a variável de ambiente ODDS_API_KEY" -ForegroundColor Yellow
    exit 1
}
